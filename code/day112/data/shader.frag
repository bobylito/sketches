...
void main() {
  ...
  for (int i = -1; i <= 1; i++) {
    for (int j = -1; j <= 1; j++) {
      ...
      point = 0.5 + 0.5 * sin(u_nFrame +
                              (snoise2(point * 0.3) * M_PI + M_PI) * point);
      float dist = length(cell + point - f_xy);
      minD = min(minD, dist);
    }
  }
  ...
  vec3 color = mix(vec3(0.8, 0.917, 0.733), vec3(0.247, 0.247, 0.266),
                   find_closest(x, y, smoothstep(0.2, 0.9, fract(minD * 2.))));
